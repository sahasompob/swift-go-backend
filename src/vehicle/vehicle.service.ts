...
function optYear(v: any) {
  ...
  if (!Number.isInteger(n) || n < 1980 || n > year + 1)
...
function toVehicleType(v: any) {
  ...
  if (!VEHICLE_TYPES.includes(s as VehicleType)) {
    throw new BadRequestException(
      `type must be one of ${VEHICLE_TYPES.join(', ')}`,
    );
  }
...
export class VehicleService {
  ...
  async registerMyVehicle(body: any) {
    ...
    const role = reqStr(body.role, 'role');
    const driverId = optNum(body.driverId, 'driverId');

    if (!driverId) throw new ForbiddenException('Unauthorized');
